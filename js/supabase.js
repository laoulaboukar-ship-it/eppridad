...
// Constantes depuis config.js (source unique de vérité)
var SUPABASE_URL      = window.CFG ? window.CFG.SUPABASE_URL      : 'https://iethhoddmztmjdhhmgsb.supabase.co';
var SUPABASE_ANON_KEY = window.CFG ? window.CFG.SUPABASE_ANON_KEY : 'sb_publishable_PnJERdL-4gumDe-wHQNLfg_HborRshv';

// EmailJS Configuration
var EMAILJS_PUBLIC_KEY  = window.CFG ? window.CFG.EMAILJS.PUBLIC_KEY  : 'S_LQPUgqU6988zXny';
var EMAILJS_SERVICE_ID  = window.CFG ? window.CFG.EMAILJS.SERVICE_ID  : 'service_5sapdz7';
var EMAILJS_TEMPLATE_ID = window.CFG ? window.CFG.EMAILJS.TEMPLATE_ID : 'template_6iuy2mm';
...
// Vérifie un mot de passe contre un hash stocké — supporte SHA-256 (64 chars) et simpleHash legacy
...
function getSession(){try{const v=localStorage.getItem(SESSION_KEY);return v?JSON.parse(v):null;}catch(e){return null;}}
...
function checkRateLimit(matricule) {
  ...
  var MAX_ATTEMPTS = 5;
  if (!_loginAttempts[key]) _loginAttempts[key] = { count: 0, first: now };
  var rec = _loginAttempts[key];
  if (now - rec.first > WINDOW_MS) {
    _loginAttempts[key] = { count: 0, first: now };
    rec = _loginAttempts[key];
  }
  rec.count++;
  if (rec.count > MAX_ATTEMPTS) {
    var waitMin = Math.ceil((WINDOW_MS - (now - rec.first)) / 60000);
    throw new Error('Trop de tentatives de connexion. Réessayez dans ' + waitMin + ' minute(s) ou contactez EPPRIDAD au +227 99 85 15 32.');
  }
}
function resetRateLimit(matricule){
  var key = (matricule||'').toUpperCase();
