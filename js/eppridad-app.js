...
function imprimerCertificat(num,nom,form,mention,score,date){
  ...
  const qrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=160x160&color=0D3B1A&bgcolor=F9F6EF&data=${encodeURIComponent(verifyUrl)}`;
  const w = window.open('','_blank','width=1100,height=820');
  w.document.write(`<!DOCTYPE html><html lang="fr"><head><meta charset="UTF-8">
  <title>Certificat EPPRIDAD — ${escH(nom)}</title>
  <link href="https://fonts.googleapis.com/css2?family=Cormorant+Garamond:ital,wght@0,400;0,600;0,700;1,400;1,600&family=IM+Fell+English:ital@0;1&family=Cinzel:wght@400;600;700&family=Lato:wght@300;400;700&display=swap" rel="stylesheet">
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    html,body{width:100%;height:100%}
    body{background:#e8e0d0;font-family:'Lato',sans-serif;display:flex;flex-direction:column;align-items:center;padding:24px 16px 40px}
    @media print{
      @page{size:A4 landscape;margin:0}
      body{background:#fff;padding:0}
      .no-print{display:none!important}
      .cert-outer{box-shadow:none!important;border-radius:0!important}
    }

    /* Boutons */
    .actions{display:flex;gap:12px;margin-bottom:20px;flex-wrap:wrap;justify-content:center}
    .btn-action{display:flex;align-items:center;gap:8px;background:#0D3B1A;color:#D4AF37;border:1px solid rgba(212,175,55,.4);border-radius:10px;padding:10px 22px;font-size:13px;font-weight:700;cursor:pointer;font-family:'Lato',sans-serif;letter-spacing:.3px;transition:all .2s}
    .btn-action:hover{background:#1a5d2e;box-shadow:0 4px 16px rgba(13,59,26,.4)}
    .btn-action.secondary{background:transparent;color:#0D3B1A;border-color:#0D3B1A}

    /* Conteneur principal */
    .cert-outer{
      width:1020px;max-width:100%;
      background:linear-gradient(165deg,#FDFAF3 0%,#F9F5E8 40%,#F5EFD8 100%);
      border-radius:4px;
      box-shadow:0 32px 80px rgba(0,0,0,.35),0 2px 0 rgba(212,175,55,.4) inset;
      padding:14px;
      position:relative;
    }

    /* Cadre double — bordure externe dorée */
    .cert-frame-outer{
      border:2.5px solid #B8940A;
      border-radius:2px;
      padding:10px;
      position:relative;
    }
    /* Cadre interne fin */
    .cert-frame-inner{
      border:1px solid rgba(184,148,10,.5);
      border-radius:1px;
    ...
    /* Ornements coins SVG */
    .corner{position:absolute;width:64px;height:64px;opacity:.7}
    ...
    .watermark{
      ...
      color:rgba(13,59,26,.04);letter-spacing:16px;
    ...
    .cert-header{
      ...
      padding-bottom:18px;
      border-bottom:1px solid rgba(184,148,10,.3);
      margin-bottom:22px;
    ...
    .cert-seal{
      ...
      border:2px solid #B8940A;border-radius:50%;
      background:radial-gradient(circle,#0D3B1A,#071f0e);
      display:flex;align-items:center;justify-content:center;
      flex-direction:column;gap:2px;
      box-shadow:0 0 0 3px rgba(184,148,10,.2),0 4px 12px rgba(0,0,0,.3);
    }
    .cert-seal-ico{font-size:22px;line-height:1}
    .cert-seal-txt{font-family:'Cinzel',serif;font-size:7px;color:#D4AF37;letter-spacing:1px;font-weight:600;text-align:center;line-height:1.2}
    .cert-brand{line-height:1}
    .cert-brand-name{font-family:'Cinzel',serif;font-size:22px;font-weight:700;color:#0D3B1A;letter-spacing:2px}
    .cert-brand-full{font-size:9.5px;color:#5a4a20;letter-spacing:.5px;margin-top:2px;font-style:italic}
    .cert-brand-loc{font-size:8.5px;color:#8a7a50;letter-spacing:1.5px;text-transform:uppercase;margin-top:1px}
    .cert-header-right{text-align:right}
    .cert-ref-lbl{font-size:8px;color:#8a7a50;letter-spacing:2px;text-transform:uppercase;margin-bottom:4px}
    .cert-ref-num{
      font-family:'Cinzel',serif;font-size:13px;font-weight:600;color:#0D3B1A;
      background:rgba(13,59,26,.06);border:1px solid rgba(184,148,10,.4);
      border-radius:4px;padding:4px 12px;letter-spacing:1px;
    }

    /* Corps central */
    .cert-body{text-align:center;padding:4px 20px 18px;position:relative;z-index:1}
    .cert-type{
      font-family:'Cinzel',serif;font-size:11px;font-weight:600;
      color:#B8940A;letter-spacing:4px;text-transform:uppercase;
      margin-bottom:10px;
    }
    .cert-title-deco{
    ...
    .cert-deco-line{height:1px;width:80px;background:linear-gradient(to right,transparent,#B8940A)}
    .cert-deco-line.right{background:linear-gradient(to left,transparent,#B8940A)}
    .cert-deco-diamond{color:#B8940A;font-size:12px}
    .cert-label-certifie{
      font-family:'IM Fell English',serif;font-size:14px;
      color:#5a4a20;font-style:italic;margin-bottom:8px;
    }
    .cert-nom{
    ...
    .cert-formation{
      ...
      color:#B8940A;line-height:1.3;
      margin-bottom:18px;padding:0 40px;
    }

    /* Badges */
    .cert-badges{display:flex;justify-content:center;gap:12px;flex-wrap:wrap;margin-bottom:10px}
    .cert-badge{
      background:linear-gradient(135deg,rgba(13,59,26,.08),rgba(13,59,26,.04));
      border:1px solid rgba(184,148,10,.4);
      color:#0D3B1A;border-radius:20px;
    ...
    .cert-footer{
      ...
      padding-top:18px;
      border-top:1px solid rgba(184,148,10,.3);
      position:relative;z-index:1;
    }
    .cert-sign{text-align:center}
    .cert-sign-line{
      width:150px;margin:0 auto 6px;
      border-bottom:1.5px solid #0D3B1A;
      position:relative;
    }
    .cert-sign-line::after{
      content:'✦';position:absolute;bottom:-8px;left:50%;transform:translateX(-50%);
      color:#B8940A;font-size:11px;background:#F9F5E8;padding:0 4px;
    }
    .cert-sign-name{font-family:'Cinzel',serif;font-size:11px;font-weight:600;color:#0D3B1A;margin-top:12px;letter-spacing:.5px}
    .cert-sign-role{font-size:9px;color:#8a7a50;letter-spacing:.5px;margin-top:2px}

    /* QR code central */
    .cert-qr-center{text-align:center;display:flex;flex-direction:column;align-items:center}
    .cert-qr-frame{
      border:1.5px solid #B8940A;border-radius:4px;
      padding:6px;background:#FDFAF3;
      box-shadow:0 2px 8px rgba(0,0,0,.1);
    }
    .cert-qr-frame img{width:80px;height:80px;display:block}
    .cert-qr-label{font-size:8px;color:#8a7a50;margin-top:5px;letter-spacing:.5px;text-align:center}

    .cert-date-zone{text-align:center}
    .cert-date-lbl{font-size:8px;color:#8a7a50;text-transform:uppercase;letter-spacing:1.5px;margin-bottom:4px}
    .cert-date-val{font-family:'Cormorant Garamond',serif;font-size:15px;font-weight:600;color:#0D3B1A}
    .cert-date-lieu{font-size:9px;color:#8a7a50;font-style:italic;margin-top:3px}

    /* Pied de page */
    .cert-bottom-bar{
      text-align:center;margin-top:8px;
      font-size:8px;color:#8a7a50;letter-spacing:.5px;
      position:relative;z-index:1;
    }
  </style>
  </head><body>
  <div class="actions no-print">
    <button class="btn-action" onclick="window.print()">🖨️ Imprimer / Télécharger PDF</button>
    <button class="btn-action secondary" onclick="navigator.clipboard.writeText('${verifyUrl}').then(()=>alert('Lien copié !'))">🔗 Copier le lien de vérification</button>
  </div>
  ...
  <div class="cert-outer">
    <div class="cert-frame-outer">
      <div class="cert-frame-inner">
        ...
        <svg class="corner corner-tl" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M2 2 L62 2 M2 2 L2 62" stroke="#B8940A" stroke-width="1.5"/>
          <path d="M8 8 L54 8 M8 8 L8 54" stroke="#B8940A" stroke-width=".7" stroke-dasharray="3 3"/>
          <circle cx="8" cy="8" r="3" fill="#B8940A" opacity=".6"/>
          <path d="M18 2 Q12 12 2 18" stroke="#B8940A" stroke-width=".8" fill="none"/>
          <path d="M32 2 Q20 20 2 32" stroke="#B8940A" stroke-width=".5" fill="none" opacity=".5"/>
          <circle cx="2" cy="2" r="2.5" fill="#B8940A"/>
        </svg>
        <svg class="corner corner-tr" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M2 2 L62 2 M2 2 L2 62" stroke="#B8940A" stroke-width="1.5"/>
          <path d="M8 8 L54 8 M8 8 L8 54" stroke="#B8940A" stroke-width=".7" stroke-dasharray="3 3"/>
          <circle cx="8" cy="8" r="3" fill="#B8940A" opacity=".6"/>
          <path d="M18 2 Q12 12 2 18" stroke="#B8940A" stroke-width=".8" fill="none"/>
        ...
        <svg class="corner corner-bl" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M2 2 L62 2 M2 2 L2 62" stroke="#B8940A" stroke-width="1.5"/>
          <path d="M8 8 L54 8 M8 8 L8 54" stroke="#B8940A" stroke-width=".7" stroke-dasharray="3 3"/>
          <circle cx="8" cy="8" r="3" fill="#B8940A" opacity=".6"/>
          <path d="M18 2 Q12 12 2 18" stroke="#B8940A" stroke-width=".8" fill="none"/>
        ...
        <svg class="corner corner-br" viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M2 2 L62 2 M2 2 L2 62" stroke="#B8940A" stroke-width="1.5"/>
          <path d="M8 8 L54 8 M8 8 L8 54" stroke="#B8940A" stroke-width=".7" stroke-dasharray="3 3"/>
          <circle cx="8" cy="8" r="3" fill="#B8940A" opacity=".6"/>
          <path d="M18 2 Q12 12 2 18" stroke="#B8940A" stroke-width=".8" fill="none"/>
        ...
        <!-- Filigrane -->
        <div class="watermark">EPPRIDAD</div>

        <!-- EN-TÊTE -->
        <div class="cert-header">
          <div class="cert-header-left">
            <div class="cert-seal">
              <div class="cert-seal-ico">🌱</div>
              <div class="cert-seal-txt">EPPRIDAD<br>NIGER</div>
            </div>
            <div class="cert-brand">
              <div class="cert-brand-name">EPPRIDAD</div>
              <div class="cert-brand-full">École Polytechnique Privée d'Agriculture</div>
              <div class="cert-brand-loc">Niamey · République du Niger · Est. 2024</div>
            </div>
          </div>
          <div class="cert-header-right">
            <div class="cert-ref-lbl">Référence officielle</div>
            <div class="cert-ref-num">${escH(num)}</div>
          </div>
        </div>

        <!-- CORPS -->
        <div class="cert-body">
          <div class="cert-type">Certificat de Formation Professionnelle</div>

          <div class="cert-title-deco">
            <div class="cert-deco-line"></div>
            <div class="cert-deco-diamond">✦</div>
            <div class="cert-deco-line right"></div>
          </div>

          <div class="cert-label-certifie">Certifie que</div>
          <div class="cert-nom">${escH(nom)}</div>
          <div class="cert-label-pour">a complété avec succès la formation</div>
          <div class="cert-formation">« ${escH(form)} »</div>

          <div class="cert-badges">
            <div class="cert-badge">🏆 Mention ${escH(mention)}</div>
            <div class="cert-badge">📊 Score ${score}%</div>
            <div class="cert-badge">✅ Formation certifiante EPPRIDAD</div>
            <div class="cert-badge">🌍 Niamey, Niger</div>
          </div>
        </div>
        ...
        <div class="cert-footer">
          <div class="cert-sign">
            ...
            <div class="cert-sign-name">Direction Générale</div>
            <div class="cert-sign-role">EPPRIDAD · Niamey, Niger</div>
          </div>

          <div class="cert-qr-center">
            <div class="cert-qr-frame">
              <img src="${qrUrl}" alt="QR Code vérification" onerror="this.parentElement.style.display='none'">
            </div>
            <div class="cert-qr-label">Scanner pour vérifier<br>l'authenticité</div>
          </div>

          <div class="cert-date-zone">
            <div class="cert-date-lbl">Délivré le</div>
            <div class="cert-date-val">${escH(date)}</div>
            <div class="cert-date-lieu">Niamey, République du Niger</div>
          </div>
        </div>
