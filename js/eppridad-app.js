...
async function reinitialiserMotDePasse(matricule){
  ...
  try{
    const db2 = getDBv30();
    await db2.from('portail_comptes').update({pwd_hash: simpleHash(pwd)}).eq('matricule', matricule);
    const fb = document.getElementById('ident-feedback');
...
async function loadAdmFinances(){
  setTitle('Finances','Administration'); showPage('page-adm-finances');
  const el = document.getElementById('page-adm-finances');
  el.innerHTML = `
    <div style="font-family:'Playfair Display',serif;font-size:24px;font-weight:700;color:var(--w);margin-bottom:4px">💰 Finances & Paiements</div>
    <div style="font-size:13px;color:var(--w3);margin-bottom:20px">Suivi des accès activés et des paiements enregistrés</div>
    <div style="display:flex;gap:8px;margin-bottom:20px">
      <button onclick="loadAdmFinances()" style="background:rgba(255,255,255,.07);border:1px solid rgba(255,255,255,.12);border-radius:9px;padding:8px 16px;font-size:13px;font-weight:700;color:var(--w2);cursor:pointer;font-family:inherit">🔄 Actualiser</button>
    </div>
    <div id="finances-kpi" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:12px;margin-bottom:24px"></div>
    <div style="font-size:13px;font-weight:800;color:var(--or);text-transform:uppercase;letter-spacing:.8px;margin-bottom:12px">📋 Détail des accès et paiements</div>
    <div id="finances-table">Chargement…</div>`;

  try{
    const db2 = getDBv30();
    const [{data:acces},{data:formations},{data:comptes}] = await Promise.all([
      db2.from('acces_formations').select('*').order('created_at',{ascending:false}),
      db2.from('formations_enligne').select('id,titre,prix_fcfa,emoji'),
      db2.from('portail_comptes').select('matricule,nom_complet,email'),
    ]);

    const formMap = {}; (formations||[]).forEach(f=>{formMap[f.id]=f;});
    const cptMap  = {}; (comptes||[]).forEach(c=>{cptMap[c.matricule]=c;});

    const actifs   = (acces||[]).filter(a=>a.actif);
    const totalCA  = actifs.reduce((s,a)=>s+(a.montant_paye||0),0);
    const avecPrix = actifs.filter(a=>a.montant_paye>0);
    const wave     = actifs.filter(a=>(a.moyen_paiement||'').toLowerCase().includes('wave')).length;
    const airtel   = actifs.filter(a=>(a.moyen_paiement||'').toLowerCase().includes('airtel')).length;
    const autres   = actifs.filter(a=>!(a.moyen_paiement||'').toLowerCase().includes('wave')&&!(a.moyen_paiement||'').toLowerCase().includes('airtel')).length;

    document.getElementById('finances-kpi').innerHTML = [
      {ico:'👥',lbl:'Accès actifs',val:actifs.length,c:'var(--or)'},
      {ico:'💵',lbl:'CA enregistré',val:fmt(totalCA)+' FCFA',c:'#81c784'},
      {ico:'📱',lbl:'Wave',val:wave,c:'#64b5f6'},
      {ico:'📲',lbl:'Airtel/Moov',val:airtel,c:'#ffb74d'},
      {ico:'📋',lbl:'Paiements notés',val:avecPrix.length,c:'#ce93d8'},
    ].map(k=>`<div style="background:rgba(255,255,255,.04);border:1px solid rgba(255,255,255,.1);border-radius:14px;padding:18px;text-align:center">
      <div style="font-size:26px;margin-bottom:6px">${k.ico}</div>
      <div style="font-family:'Playfair Display',serif;font-size:22px;font-weight:700;color:${k.c}">${k.val}</div>
      <div style="font-size:11px;color:var(--w3);margin-top:4px">${k.lbl}</div>
    </div>`).join('');

    document.getElementById('finances-table').innerHTML =
      `<div class="tbl-wrap"><table><thead><tr>
        <th>Matricule</th><th>Nom</th><th>Formation</th>
        <th>Montant</th><th>Moyen</th><th>Réf.</th><th>Date accès</th><th>Expire</th><th>Actions</th>
      </tr></thead><tbody>${
        (acces||[]).map((a,i)=>{
          const f = formMap[a.formation_id];
          const c = cptMap[a.matricule];
          const dateDebut = a.date_debut ? new Date(a.date_debut).toLocaleDateString('fr-FR') : '—';
          const dateFin   = a.date_fin   ? new Date(a.date_fin).toLocaleDateString('fr-FR')   : '—';
          const bg = i%2===0?'rgba(255,255,255,.02)':'rgba(255,255,255,.04)';
          return `<tr style="background:${bg}">
            <td style="font-size:11px;font-weight:700;color:var(--or);font-family:monospace">${escH(a.matricule)}</td>
            <td style="font-size:12px;color:var(--w)">${escH(c?.nom_complet||'—')}</td>
            <td style="font-size:12px;color:var(--w2)">${f?escH(f.emoji+' '+f.titre):'—'}</td>
            <td style="font-size:12px;font-weight:700;color:${a.montant_paye?'#81c784':'rgba(255,255,255,.3)'}">${a.montant_paye?fmt(a.montant_paye)+' FCFA':'—'}</td>
            <td style="font-size:11px;color:var(--w3)">${escH(a.moyen_paiement||'—')}</td>
            <td style="font-size:10px;color:var(--w3);font-family:monospace">${escH(a.ref_paiement||'—')}</td>
            <td style="font-size:11px;color:var(--w3)">${dateDebut}</td>
            <td style="font-size:11px;color:${a.actif?'#81c784':'rgba(255,255,255,.3)'}">${dateFin}</td>
            <td><button onclick="ouvrirPaiementModal('${a.id}','${escH(a.matricule)}','${escH(c?.nom_complet||'')}','${escH(f?.titre||'')}')" style="background:rgba(201,168,76,.15);color:var(--or);border:1px solid rgba(201,168,76,.3);border-radius:7px;padding:4px 10px;font-size:11px;font-weight:700;cursor:pointer;font-family:inherit">✏️ Paiement</button></td>
          </tr>`;
        }).join('')
      }</tbody></table></div>`;
  }catch(e){
    document.getElementById('finances-table').innerHTML=`<div style="color:var(--rd);padding:20px">Erreur : ${e.message}</div>`;
  }
}
...
async function creerApprenantenLigne(){
  ...
  try{
    ...
    await db2.from('portail_comptes').upsert({
      matricule:mat, pwd_hash:simpleHash(pwd),
