...
// Remplace le mot de passe par défaut prévisible 'eppridad2025'
// Génère 8 caractères mixtes (lettres + chiffres), lisibles et mémorisables
...
async function afficherCertificat(){
  ...
  if(!cert){
    ...
    // ── Nom sur le certificat : nom_certificat (enfant) en priorité, sinon nom du compte ──
    const { data:accesData } = await db.from('acces_formations')
      .select('nom_certificat')
      .eq('matricule',_s.matricule)
      .eq('formation_id',_cours.formationId)
      .single();
    const nomCert = (accesData && accesData.nom_certificat && accesData.nom_certificat.trim())
      ? accesData.nom_certificat.trim()
      : (_s.nom || _s.matricule);

    const { data:newCert } = await db.from('certificats').insert({
      matricule:_s.matricule, formation_id:_cours.formationId,
      numero:num, nom_apprenant:nomCert,
      score_final:Math.round(avg*10)/10, mention, valide:true,
      date_emission:new Date().toISOString()
    }).select().single();
    cert = newCert || {numero:num, nom_apprenant:nomCert, mention, score_final:Math.round(avg*10)/10, date_emission:new Date().toISOString()};
  }

  // ── Si nom_certificat jamais saisi pour cette formation, proposer de le renseigner ──
  let nomAffiche = cert.nom_apprenant || _s.nom;
  const formTitre = document.getElementById('cs-formation-titre')?.textContent || '';
  const isF29 = formTitre.toLowerCase().includes('enfant') || formTitre.toLowerCase().includes('maison');
  ...
  const nbModules = _cours.modules ? _cours.modules.length : 4;

  container.innerHTML = `
  <div style="padding:24px 0">
    ${isF29 ? `
    <div id="nom-enfant-banner" style="background:rgba(212,175,55,.1);border:1px solid rgba(212,175,55,.3);border-radius:12px;padding:16px 20px;margin-bottom:20px;text-align:center">
      <div style="font-size:13px;color:rgba(255,255,255,.7);margin-bottom:10px">🎓 Ce certificat sera au nom de :</div>
      <div style="font-size:20px;font-weight:800;color:#d4af37;margin-bottom:12px" id="nom-cert-affiche">${escH(nomAffiche)}</div>
      <div style="display:flex;gap:8px;justify-content:center;flex-wrap:wrap">
        <input id="nom-enfant-input" type="text" placeholder="Prénom et nom de l'enfant"
          value="${escH(nomAffiche)}"
          style="background:rgba(255,255,255,.08);border:1px solid rgba(255,255,255,.2);border-radius:8px;padding:9px 14px;font-size:14px;color:#fff;font-family:inherit;outline:none;width:240px">
        <button onclick="sauvegarderNomEnfant()" style="background:linear-gradient(135deg,#d4af37,#b8941f);color:#0a1a0a;border:none;border-radius:8px;padding:9px 18px;font-size:13px;font-weight:800;cursor:pointer;font-family:inherit">✅ Confirmer</button>
      </div>
      <div id="nom-enfant-fb" style="font-size:12px;color:#81c784;margin-top:8px;display:none">✅ Nom mis à jour !</div>
    </div>` : ''}
    <div class="cert-card">
      ...
      <div class="cert-body">
        ...
        <div class="cert-name" id="cert-nom-live">${escH(nomAffiche)}</div>
        ...
        <div class="cert-actions">
          <button class="btn-cert btn-cert-dl" id="btn-telecharger-cert"
            onclick="imprimerCertificat('${escH(cert.numero)}',document.getElementById('cert-nom-live').textContent,'${escH(formTitre)}','${escH(cert.mention||'Bien')}','${cert.score_final||0}','${dateStr}',${nbModules})">📄 Télécharger le certificat</button>
  ...
  // Stocker les infos pour la fonction de sauvegarde
  window._certCurrent = { matricule:_s.matricule, formationId:_cours.formationId, numero:cert.numero };
}

async function sauvegarderNomEnfant(){
  const input = document.getElementById('nom-enfant-input');
  const fb = document.getElementById('nom-enfant-fb');
  const nomLive = document.getElementById('cert-nom-live');
  if(!input || !input.value.trim()) return;
  const nom = input.value.trim();
  try{
    const db = getDBv30();
    // Mettre à jour nom_certificat dans acces_formations
    await db.from('acces_formations')
      .update({nom_certificat: nom})
      .eq('matricule', window._certCurrent.matricule)
      .eq('formation_id', window._certCurrent.formationId);
    // Mettre à jour nom_apprenant dans certificats
    await db.from('certificats')
      .update({nom_apprenant: nom})
      .eq('numero', window._certCurrent.numero);
    // Mettre à jour l'affichage en temps réel
    if(nomLive) nomLive.textContent = nom;
    document.getElementById('nom-cert-affiche').textContent = nom;
    if(fb){ fb.style.display='block'; setTimeout(()=>fb.style.display='none', 3000); }
  }catch(e){ if(fb){ fb.textContent='❌ Erreur : '+e.message; fb.style.color='#ef9a9a'; fb.style.display='block'; } }
}

function imprimerCertificat(num,nom,form,mention,score,date,nbModules){
  nbModules = nbModules || 4;
  const verifyUrl = `https://www.eppridad.com/verifier.html?cert=${encodeURIComponent(num)}`;
  const qrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=150x150&color=07120e&bgcolor=C9A84C&data=${encodeURIComponent(verifyUrl)}`;
  const logoUrl = 'https://www.eppridad.com/images/logo_eppridad.png';
  const w = window.open('','_blank','width=1100,height=800');
  if(!w){ alert('Votre navigateur a bloqué l\'ouverture. Autorisez les popups pour eppridad.com.'); return; }
  w.document.write(`<!DOCTYPE html><html lang="fr"><head>
  <meta charset="UTF-8">
  <title>Certificat EPPRIDAD — ${nom}</title>
  <link href="https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,400;0,700;0,900;1,400;1,700&family=Cormorant+Garamond:wght@400;600;700&family=Outfit:wght@400;600;700&display=swap" rel="stylesheet">
  <style>
    *{box-sizing:border-box;margin:0;padding:0}
    body{background:#e8e0d0;font-family:'Outfit',sans-serif;padding:24px;min-height:100vh}
    @media print{
      @page{size:A4 landscape;margin:0}
      body{background:white;padding:0}
      .no-print{display:none!important}
      .cert-page{border-radius:0!important;box-shadow:none!important;min-height:100vh}
    }

    /* ── Actions ── */
    .actions{display:flex;justify-content:center;gap:12px;margin-bottom:20px;flex-wrap:wrap}
    .actions button{padding:11px 22px;border-radius:50px;font-size:14px;font-weight:700;cursor:pointer;border:none;font-family:inherit;transition:all .2s}
    .btn-print{background:#07120e;color:#C9A84C;border:1px solid rgba(201,168,76,.4)}
    .btn-copy{background:rgba(7,18,14,.08);color:#07120e;border:1px solid rgba(7,18,14,.2)}
    .btn-print:hover{background:#16503f}

    /* ── Certificat ── */
    .cert-page{
      max-width:960px;margin:0 auto;
      background:linear-gradient(145deg,#050e09 0%,#09231a 35%,#0d3526 65%,#071810 100%);
      border-radius:20px;
      padding:0;
      position:relative;
      overflow:hidden;
      box-shadow:0 32px 80px rgba(0,0,0,.55), inset 0 0 0 1px rgba(201,168,76,.25);
    }

    /* Bordure dorée ornementale */
    .cert-page::before{
    ...
    .cert-inner{position:relative;z-index:1;padding:40px 52px}
    ...
    .cert-logo-zone{display:flex;align-items:center;gap:16px}
    .cert-logo-img{width:64px;height:64px;object-fit:contain;filter:drop-shadow(0 2px 8px rgba(0,0,0,.4))}
    .cert-logo-fallback{width:64px;height:64px;background:rgba(201,168,76,.15);border:1.5px solid rgba(201,168,76,.35);border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:30px}
    .cert-school-name{font-family:'Playfair Display',serif;font-size:20px;color:#C9A84C;font-weight:700;line-height:1.1}
    .cert-school-sub{font-size:9.5px;color:rgba(255,255,255,.35);letter-spacing:2px;text-transform:uppercase;margin-top:3px}
    .cert-school-sub2{font-size:9px;color:rgba(255,255,255,.25);letter-spacing:1px;text-transform:uppercase;margin-top:2px}
    .cert-num-zone{text-align:right}
    .cert-num-lbl{font-size:9px;color:rgba(255,255,255,.25);letter-spacing:2px;text-transform:uppercase;margin-bottom:3px}
    .cert-num-val{font-size:12px;color:rgba(201,168,76,.6);font-weight:700;letter-spacing:1px;font-family:'Outfit',monospace}

    /* ── Corps ── */
    .cert-body{text-align:center;padding:10px 0 22px}
    .cert-certifie{
    ...
    .cert-accompli{font-size:13px;color:rgba(255,255,255,.45);margin-bottom:10px}
    .cert-formation{
      font-family:'Cormorant Garamond',serif;
      font-size:24px;font-weight:700;
      color:#C9A84C;font-style:italic;
      margin-bottom:22px;padding:0 48px;line-height:1.35
    }
    ...
    .cert-badge.gold{background:rgba(201,168,76,.15);color:#C9A84C;border-color:rgba(201,168,76,.35)}
    ...
    .cert-sign{text-align:center;min-width:140px}
    .cert-sign-line{width:120px;height:1px;background:rgba(255,255,255,.25);margin:0 auto 7px}
    .cert-sign-name{font-size:11px;color:rgba(255,255,255,.6);font-weight:700;text-transform:uppercase;letter-spacing:.5px}
    .cert-sign-title{font-size:9px;color:rgba(255,255,255,.25);margin-top:2px}
    .cert-date-zone{text-align:center}
    .cert-date-lbl{font-size:9px;color:rgba(255,255,255,.25);text-transform:uppercase;letter-spacing:1.5px;margin-bottom:4px}
    .cert-date-val{font-size:14px;color:rgba(255,255,255,.55);font-weight:600}
    .cert-qr{text-align:center}
    .cert-qr-img{width:80px;height:80px;border-radius:8px;background:#C9A84C;padding:5px;display:block}
    .cert-qr-txt{font-size:8.5px;color:rgba(255,255,255,.2);margin-top:5px;max-width:100px;line-height:1.4}

    .cert-watermark-bottom{
  ...
  <div class="actions no-print">
    <button class="btn-print" onclick="window.print()">🖨️ Télécharger en PDF</button>
    <button class="btn-copy" onclick="navigator.clipboard.writeText('${verifyUrl}').then(()=>alert('✅ Lien de vérification copié !')).catch(()=>prompt('Lien de vérification :','${verifyUrl}'))">🔗 Copier le lien QR</button>
    <button class="btn-copy" onclick="window.close()">✕ Fermer</button>
  </div>
    ...
    <div class="cert-inner">
      ...
      <div class="cert-header">
        <div class="cert-logo-zone">
          <img class="cert-logo-img" src="${logoUrl}"
               alt="Logo EPPRIDAD"
               onerror="this.style.display='none';this.nextElementSibling.style.display='flex'">
          <div class="cert-logo-fallback" style="display:none">🎓</div>
          <div>
            <div class="cert-school-name">EPPRIDAD</div>
            <div class="cert-school-sub">École Polytechnique Privée pour le</div>
            <div class="cert-school-sub2">Développement Agricole Durable · Niamey, Niger</div>
          </div>
        </div>
        <div class="cert-num-zone">
          <div class="cert-num-lbl">Numéro de certificat</div>
          <div class="cert-num-val">${num}</div>
        </div>
      </div>

      <!-- CORPS -->
      <div class="cert-body">
        <div class="cert-certifie">Certificat officiel de formation</div>
        <div class="cert-type">Certifie que</div>
        <div class="cert-nom">${nom}</div>
        <div class="cert-nom-line"></div>
        <div class="cert-accompli">a complété avec succès la formation</div>
        <div class="cert-formation">« ${form} »</div>
        <div class="cert-badges">
          <div class="cert-badge gold">🏆 Mention : ${mention}</div>
          <div class="cert-badge gold">📊 Score : ${score}%</div>
          <div class="cert-badge">${nbModules} modules validés</div>
          <div class="cert-badge">✅ Certifié EPPRIDAD</div>
        </div>
      </div>
      ...
      <div class="cert-footer">
        ...
        <div class="cert-qr">
          <img class="cert-qr-img" src="${qrUrl}" alt="QR Code vérification"
               onerror="this.src='https://chart.googleapis.com/chart?chs=150x150&cht=qr&chl=${encodeURIComponent(verifyUrl)}'">
      ...
      <div class="cert-watermark-bottom">
        www.eppridad.com · eppridad@gmail.com · +227 99 85 15 32 · Vérifiable sur : ${verifyUrl}
      </div>

    </div>
  </div>
  <script>
    // Déclencher l'impression automatiquement si on est sur mobile
    // (évite l'étape manuelle de cliquer le bouton)
...
function openIdentifiantsModal(matricule, nomComplet, telephone, email){
  ...
  body.innerHTML = `
    ...
    <div style="margin-bottom:14px">
      ...
      <div style="display:flex;gap:8px">
        <input id="ident-pwd" value="${genererMotDePasseAleatoire()}" style="flex:1;background:rgba(255,255,255,.07);border:1px solid rgba(255,255,255,.15);border-radius:9px;padding:10px 12px;font-size:14px;font-weight:700;color:#fff;font-family:monospace">
  ...
  modal.innerHTML=`<div style="background:var(--v1,#0f2818);border:1px solid rgba(201,168,76,.25);border-radius:16px;max-width:480px;width:100%;padding:0;overflow:hidden;max-height:90vh;overflow-y:auto">
    ...
    <div style="padding:22px;display:grid;gap:12px">
      ${[
        ...
        ['enl-pwd','Mot de passe initial',genererMotDePasseAleatoire(),'text'],
