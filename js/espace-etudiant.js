...
// ── COMPATIBILITÉ EPPRIDAD-APP.JS ──
// Ces stubs sont écrasés par eppridad-app.js qui se charge après
if(typeof toast === 'undefined')              window.toast = function(m,d){console.log('[toast]',m);};
if(typeof fmt === 'undefined')                window.fmt = function(n){return Number(n).toLocaleString('fr-FR');};
if(typeof showError === 'undefined')          window.showError = function(m){console.error(m);};
if(typeof showSuccess === 'undefined')        window.showSuccess = function(m){console.log(m);};
if(typeof clearSession === 'undefined')       window.clearSession = function(){try{localStorage.removeItem('eppr_session_v30');}catch(e){}};
if(typeof aPanel === 'undefined')             window.aPanel = function(n,b){console.log('aPanel',n);};
if(typeof loadAdmInscriptions === 'undefined') window.loadAdmInscriptions = function(){console.log('loadAdmInscriptions non chargé');};
if(typeof updateAdminClock === 'undefined')    window.updateAdminClock = function(){};
// ── FIN COMPATIBILITÉ ──
...
function doAdminLogin(){
  ...
  if(id==='ADMIN'&&simpleHash(pwd)===getAdminHash()){
    ...
    showPageV27('admin-page');      // Afficher la page AVANT de charger les données
...
async function doRegister(){
  ...
  try {
    ...
    showSendModal('Demande accès — '+id, msg, ()=>switchTabV27('pending'));
  } catch(e) {
    err.textContent=e.message;err.classList.add('show');
  } finally {
...
async function loadStudentDashboard(matricule){
  ...
  showLoadingOverlayV27(true,'Chargement de votre espace…');
  try {
    // 1. Infos étudiant
...
// ── Overlay chargement ────────────────────────────────────────
function showLoadingOverlayV27(show, msg=''){
...
function renderStudentDashboard(e){
  ...
  ].map(s=>`<div class="hs" onclick="sPanelV27('${s.panel}',null)"><div class="hs-val">${s.v}</div><div class="hs-lbl">${s.l}</div></div>`).join('');

  sPanelV27('accueil',null);
}

// ════════════════════════════════════════════════════════════
//  PANNEAUX ÉTUDIANT
// ════════════════════════════════════════════════════════════
function sPanelV27(name,btn){
...
function buildAccueilEnligneHTML(e){
  ...
  const cards = [
    ...
    {ico:'📚',lbl:'Bibliothèque',sub:'Ressources PDF',fn:"sPanelV27('library',null)"},
    {ico:'💬',lbl:'Messages',sub:'Notifications',fn:"sPanelV27('messages',null)"},
    {ico:'👤',lbl:'Mon compte',sub:'Paramètres',fn:"sPanelV27('compte',null)"}
  ];
  const steps = [
    ...
    if(isEnligne){
      ...
      <div style="margin-bottom:16px">
        ...
        <div class="quick-access">
          ...
          <div class="qa-card" onclick="sPanelV27('library',null)">
            <div class="qa-icon">📚</div><div class="qa-label">Documents</div>
          </div>
          <div class="qa-card" onclick="sPanelV27('messages',null)">
            <div class="qa-icon">💬</div><div class="qa-label">Messages</div>
          </div>
          <div class="qa-card" onclick="sPanelV27('compte',null)">
      ...
      <div style="margin-bottom:16px">
        <div style="font-size:11px;font-weight:800;color:var(--text3);text-transform:uppercase;letter-spacing:.7px;margin-bottom:10px">Accès rapide</div>
        <div class="quick-access">
          <div class="qa-card" onclick="sPanelV27('notes',null)"><div class="qa-icon">📊</div><div class="qa-label">Mes Notes</div></div>
          <div class="qa-card" onclick="sPanelV27('bulletin',null)"><div class="qa-icon">📋</div><div class="qa-label">Bulletin</div></div>
          <div class="qa-card" onclick="sPanelV27('conseils',null)"><div class="qa-icon">🤖</div><div class="qa-label">Conseils IA</div></div>
          <div class="qa-card" onclick="sPanelV27('scolarite',null)"><div class="qa-icon">💳</div><div class="qa-label">Scolarité</div></div>
        </div>
      </div>`;
    }
  }
...
// ════════════════════════════════════════════════════════════
function showToastV27(msg, color) {
...
async function loadAdminDashboard(){
  showLoadingOverlayV27(true,'Chargement administration…');
  ...
  try{
    ...
    renderAdminDashboard(window._adminData);
    showToastV27('⚠️ Certaines données n\'ont pas pu être chargées: '+e.message, 5000);
  }
}
...
function renderAdminDashboard(data){
  ...
  const kpiCards = [
    {
      ...
      action:"aPanelV27('inscriptions',null)"
    },
    {
      ...
      action:"aPanelV27('commandes',null)"
    },
    {
      ...
      action:"aPanelV27('acces_el',null)"
    },
    {
      ...
      action:"aPanelV27('students',null)"
    },
    {
  ...
  const _as=document.getElementById('adminStats'); if(!_as) return;
  _as.innerHTML = kpiCards.map(k=>`
  ...
  if(typeof updateAdminClock === 'function') updateAdminClockV27();
...
function exportRevenusCSV(){
  ...
  showToastV27('Export CSV téléchargé !');
}
...
document.addEventListener('DOMContentLoaded',async()=>{
  ...
  if(new URLSearchParams(window.location.search).get('reset')==='1'){
    ...
    showPageV27('auth-page');
    showToastV27('Session réinitialisée. Reconnectez-vous.','#1a5d1a');
    return;
  }
  // Restaurer session
  ...
  if(sess){
    _sessionUser=sess;
    if(sess.role==='admin'){await loadAdminDashboard();showPageV27('admin-page');}
    ...
    else{clearSession();showPageV27('auth-page');}
  }
  // Clavier
  document.getElementById('loginPwd')?.addEventListener('keydown',e=>{if(e.key==='Enter')doLoginV27();});
...
// Override sPanel V27 pour ajouter l'historique
const _originalSPanel = typeof sPanelV27 !== 'undefined' ? sPanelV27 : function(){};
...
async function saveProduct() {
  ...
  try {
    if (editingProductId) {
      ...
      showToastV27('✅ Produit mis à jour !');
    } else {
      await sb.insert('produits_boutique', productData);
      showToastV27('✅ Produit publié dans la boutique !');
    }
    cancelAddProduct();
    loadProducts();
  } catch(err) {
...
async function toggleProductDispo(id, currentState) {
  try {
    ...
    showToastV27(!currentState ? '✅ Produit affiché dans la boutique' : '⏸ Produit masqué de la boutique');
    loadProducts();
  ...
  modal.innerHTML = `
    <div style="background:#fff;border-radius:20px;padding:28px 30px;max-width:460px;width:100%;box-shadow:0 24px 60px rgba(0,0,0,.3);animation:fadeUp .25s ease">
      ...
      <div style="display:flex;flex-direction:column;gap:10px">
        ...
        <div style="display:flex;gap:8px">
          <button onclick="navigator.clipboard&&navigator.clipboard.writeText('Identifiant: ${matricule}\nMot de passe: ${pwd}\nPortail: https://www.eppridad.com/espace-etudiant.html').then(()=>showToastV27('✅ Identifiants copiés !'))"
...
async function _DEAD_quickActiverAcces(reference, prenom, nom, tel, email, formation_titre){
  ...
  try{
    ...
    showLoadingOverlayV27(false);
    showToastV27('Erreur activation: '+err.message, '#e53935');
    console.error('quickActiverAcces error:', err);
    // Fallback manuel
    aPanelV27('acces_el',null);
    setTimeout(()=>{
      ...
      showToastV27('Complétez manuellement et cliquez Activer');
    },300);
  }
}
        ...
        <div style="display:flex;align-items:flex-start;gap:14px;flex-wrap:wrap">
          ...
          <div style="flex:1;min-width:220px">
            <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px;flex-wrap:wrap">
              <div style="font-size:15px;font-weight:800;color:#fff">${i.prenom||''} ${i.nom||''}</div>
            ...
            <div style="display:flex;flex-wrap:wrap;gap:10px;margin-bottom:8px">
              <div style="font-size:12.5px;color:rgba(255,255,255,.7);display:flex;align-items:center;gap:5px">
                <span>📞</span> <a href="tel:${i.telephone||''}" style="color:#81c784;font-weight:600;text-decoration:none">${i.telephone||'—'}</a>
              </div>
              ${i.email?`<div style="font-size:12.5px;color:rgba(255,255,255,.7);display:flex;align-items:center;gap:5px"><span>✉️</span> <a href="mailto:${i.email}" style="color:#81c784;font-weight:600;text-decoration:none">${i.email}</a></div>`:''}
              ${i.ville?`<div style="font-size:12px;color:rgba(255,255,255,.45)">📍 ${i.ville}</div>`:''}
            </div>

            ${i.resume?`<div style="font-size:12.5px;color:rgba(255,255,255,.75);background:rgba(255,255,255,.06);border-radius:8px;padding:8px 12px;margin-bottom:6px;border-left:3px solid var(--or)">${i.resume}</div>`:''}
            ${i.message?`<div style="font-size:11.5px;color:rgba(255,255,255,.6);margin-bottom:6px;font-style:italic;background:rgba(201,168,76,.08);border-radius:6px;padding:6px 10px;border-left:3px solid rgba(201,168,76,.4)">"${i.message.substring(0,120)}${i.message.length>120?'…':''}"</div>`:''}
            ${i.note_admin?`<div style="font-size:11.5px;color:#90caf9;margin-bottom:6px;background:rgba(25,118,210,.12);border-radius:6px;padding:6px 10px">📝 ${i.note_admin}</div>`:''}

            <div style="font-size:10.5px;color:rgba(255,255,255,.4);display:flex;gap:12px;flex-wrap:wrap">
              <span>Réf: <strong style="color:rgba(255,255,255,.6)">${i.reference||'—'}</strong></span>
              <span>📅 ${dateStr}</span>
...
async function updateInscriptionStatus(id, status) {
  try {
    ...
    toast('Statut mis à jour : ' + (labels[status]||status), 3000);
    loadInscriptionsBadge();
    // Recharger la liste pour que le bouton disparaisse
    if(typeof loadInscriptions==='function') loadInscriptions();
    else if(typeof loadAdmInscriptions==='function') loadAdmInscriptions();
  } catch(e) {
...
async function updateCommandeStatus(id, status) {
  try {
    ...
    showToastV27('Commande mise à jour → ' + status);
    loadCommandes();
  } catch(e) { alert('Erreur: ' + e.message); }
}
...
setInterval(updateAdminClockV27,30000);
...
async function addModule(formId){
  ...
  if(!titre){showToastV27('⚠️ Le titre est obligatoire.');return}
  try{
...
async function addRessource(formId,modId){
  ...
  if(!titre){showToastV27('⚠️ Titre obligatoire.');return}
  try{
...
async function addQuizQuestion(formId,modId){
  ...
  if(!question||!a||!b){showToastV27('⚠️ Question, options A et B sont obligatoires.');return}
  try{
...
async function giveAcces(){
  ...
  if(!matricule||!formId){showToastV27('⚠️ Matricule et formation obligatoires.');return}
  ...
  try{
    ...
    if(email && typeof emailAccesAccorde==='function'){
      emailAccesAccorde(email, nomComplet||matricule, matricule, formationTitre, pwd, expiry?expiry.toISOString():null)
        .then(ok=>{ if(ok) showToastV27('📧 Email de confirmation envoyé !','#1a5d4a'); })
        .catch(()=>{});
    }
...
async function loadAcesList(){
  ...
  if(!container)return;
  container.innerHTML='<div style="text-align:center;padding:24px;color:rgba(255,255,255,.45);font-size:13px">Chargement…</div>';
  try{
    // Requêtes séquentielles (pas de Promise.all avec SDK)
    const acces    = await sb.select('acces_formations',{order:'created_at.desc',limit:200});
    const formations = await sb.select('formations_enligne',{select:'id,titre,emoji,publie'});
    const progression = await sb.select('progression_apprenant',{select:'matricule,formation_id,complete'}).catch(()=>[]);
    const modules  = await sb.select('modules_cours',{select:'id,formation_id,publie'}).catch(()=>[]);

    const fMap={};(formations||[]).forEach(f=>{fMap[f.id]=f;});

    // Calculer progression par (matricule, formation_id)
    const progMap={};
    (progression||[]).forEach(p=>{
      const k=p.matricule+'|'+p.formation_id;
      if(!progMap[k])progMap[k]={total:0,done:0};
      progMap[k].total++;
      if(p.complete)progMap[k].done++;
    });
    // Calculer total modules publiés par formation
    const modCount={};
    (modules||[]).forEach(m=>{if(m.publie!==false){modCount[m.formation_id]=(modCount[m.formation_id]||0)+1;}});

    if(!acces||!acces.length){
      container.innerHTML='<div style="text-align:center;padding:32px;color:rgba(255,255,255,.45);font-size:14px">Aucun accès accordé pour le moment.</div>';
      return;
    }

    // Grouper par formation pour l'affichage
    const CARD='background:rgba(255,255,255,.05);border:1px solid rgba(255,255,255,.1);border-radius:14px;padding:14px 18px;margin-bottom:10px;transition:all .2s';

    container.innerHTML=acces.map(a=>{
      const f=fMap[a.formation_id]||{titre:'Formation inconnue',emoji:'❓',publie:false};
      const expired=a.date_fin&&new Date(a.date_fin)<new Date();
      const actif=a.actif&&!expired;
      const statutBg=actif?'rgba(46,125,82,.25)':expired?'rgba(229,57,53,.2)':'rgba(255,255,255,.08)';
      const statutColor=actif?'#81c784':expired?'#ef9a9a':'rgba(255,255,255,.5)';
      const statutTxt=actif?'✅ Actif':expired?'⛔ Expiré':'⏸ Inactif';

      // Progression apprenant
      const pk=a.matricule+'|'+a.formation_id;
      const prog=progMap[pk];
      const totalMod=modCount[a.formation_id]||0;
      const doneMod=prog?prog.done:0;
      const pct=totalMod>0?Math.round(doneMod/totalMod*100):0;
      const progBar=totalMod>0?`
        <div style="margin-top:8px">
          <div style="display:flex;justify-content:space-between;margin-bottom:3px">
            <span style="font-size:10px;color:rgba(255,255,255,.45)">Progression</span>
            <span style="font-size:10px;font-weight:700;color:${pct===100?'#81c784':'rgba(255,255,255,.7)'}">${doneMod}/${totalMod} modules · ${pct}%</span>
          </div>
          <div style="height:4px;background:rgba(255,255,255,.1);border-radius:2px;overflow:hidden">
            <div style="height:100%;width:${pct}%;background:${pct===100?'#81c784':'var(--or)'};border-radius:2px;transition:width .5s"></div>
          </div>
        </div>`:'';

      const formationStatut=f.publie?'<span style="font-size:9px;background:rgba(46,125,82,.2);color:#81c784;border-radius:6px;padding:1px 6px;font-weight:700">PUBLIÉE</span>':'<span style="font-size:9px;background:rgba(255,255,255,.06);color:rgba(255,255,255,.4);border-radius:6px;padding:1px 6px">EN CONSTRUCTION</span>';

      const dateActiv=a.date_creation?new Date(a.date_creation).toLocaleDateString('fr-FR'):'—';
      const dateExp=a.date_fin?new Date(a.date_fin).toLocaleDateString('fr-FR'):'Illimité';

      return `<div style="${CARD}">
        <div style="display:flex;align-items:flex-start;gap:14px;flex-wrap:wrap">
          <div style="font-size:28px;flex-shrink:0;margin-top:2px">${f.emoji}</div>
          <div style="flex:1;min-width:180px">
            <div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-bottom:4px">
              <span style="font-weight:800;font-size:14px;color:#fff;font-family:'Plus Jakarta Sans',sans-serif">${a.matricule}</span>
              ${formationStatut}
            </div>
            <div style="font-size:13px;color:rgba(255,255,255,.75);margin-bottom:3px">${f.titre}</div>
            <div style="font-size:11px;color:rgba(255,255,255,.4)">Activé le ${dateActiv} · Expire : ${dateExp}</div>
            ${a.note_admin?`<div style="font-size:11px;color:rgba(201,168,76,.7);margin-top:4px;font-style:italic">📝 ${a.note_admin}</div>`:''}
            ${progBar}
          </div>
          <div style="display:flex;flex-direction:column;gap:6px;align-items:flex-end;flex-shrink:0">
            <span style="background:${statutBg};color:${statutColor};font-size:11px;padding:4px 10px;border-radius:8px;font-weight:700">${statutTxt}</span>
            <button onclick="revokeAcces('${a.id}')" style="background:rgba(229,57,53,.15);border:1px solid rgba(229,57,53,.25);border-radius:8px;padding:5px 12px;font-size:11px;font-weight:700;cursor:pointer;color:#ef9a9a;font-family:inherit">Révoquer</button>
          </div>
        </div>
      </div>`;
    }).join('');
  }catch(e){
    container.innerHTML=`<div style="color:#ef9a9a;padding:16px;font-size:13px;background:rgba(229,57,53,.1);border-radius:10px">Erreur: ${e.message}</div>`;
  }
}
      ...
      if(rows&&rows.length){
        ...
        if(email && typeof emailCorrectionExercice==='function'){
          emailCorrectionExercice(email,nomComplet,statut,note,formTitre)
            .then(ok=>{ if(ok) showToastV27('📧 Email envoyé à l\'apprenant','#1a5d4a'); });
        }
      }
    }catch(_){}
